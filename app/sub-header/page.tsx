...
const SubHeader = () => {
  ...
  return (
    <div className="xl:pl-48 lg:pl-36 md:pl-24 sm:pl-16 pl-7 shadow-md flex overflow-visible text-gray-600">
...
{/* <div className="xl:pl-48 lg:pl-36 md:pl-24 sm:pl-16 pl-7 shadow-md flex overflow-visible">
  <div className="flex gap-2 text-sm scroll-smooth snap-x">
    {categories.slice(0, visibleCount).map((category) => (
      <button
        key={category}
        className="px-4 py-2 bg-white hover:bg-gray-100 rounded-md text-sm font-medium snap-start transition duration-200 ease-in-out"
        onClick={() => handleCategoryClick(category)}
      >
        {category}
      </button>
    ...
    {categories.length > visibleCount && (
      <div className="relative">
        <button
          ref={buttonRef}
          onClick={() => setShowMore((prev) => !prev)}
          className="px-4 py-2 bg-white hover:bg-gray-100 flex items-center gap-1 rounded-md"
        >
          More
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        {showMore && (
          <div
            ref={menuRef}
            className="absolute right-0 top-full mt-2 bg-white shadow-lg w-auto max-h-[300px] overflow-y-auto border border-gray-300 rounded-md z-50"
          >
            {categories.slice(visibleCount).map((category) => (
              <button
                key={category}
                className="block h-[40px] px-4 py-2 w-full text-left hover:bg-gray-100"
                onClick={() => handleCategoryClick(category)}
              >
                {category}
              </button>
