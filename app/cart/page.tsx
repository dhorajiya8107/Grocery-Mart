...
const CartPage = () => {
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      const existingOrder = localStorage.getItem('lastOrderId');
      if (existingOrder) {
        const orderRef = doc(db, 'orders', existingOrder);
        const orderSnapshot = await getDoc(orderRef);
  
        if (orderSnapshot.exists() && orderSnapshot.data().paymentStatus === 'Pending') {
          ...
          console.log(`Order updated with ID: ${existingOrder}`);
          router.push(`/checkout?orderId=${existingOrder}`);
          return;
        } else {
      ...
      const orderRef = doc(collection(db, 'orders'), formattedOrderId);
      ...
      console.log(`Order placed with ID: ${formattedOrderId}`);
  
      localStorage.setItem('lastOrderId', formattedOrderId);
