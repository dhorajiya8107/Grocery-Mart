            ...
            >
              <div className="flex flex-col p-4 md:p-6">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-md font-bold mb-3">
                      ...
                      ₹{order.totalAmount}
                        <span className={`absolute top-5 ml-4 py-1 px-1 rounded-md text-xs
                        ${order.paymentStatus === 'Paid' ? 'bg-green-100 text-green-700'
                          : order.paymentStatus === 'Pending'
                                ? 'bg-red-100 text-red-500'
                                : 'text-green-700'}`} >
                          {/* {order.paymentStatus || 'N/A'} */}
                          {order.paymentStatus === 'Paid' ? 'Delivered' : order.paymentStatus === 'Pending' ? 'Not Paid' : 'Not Delivered'}
                        </span>
                    </p>
                ...
                <div className="hidden min-[600px]:block justify-end">
                  <button
                    onClick={() => router.push(`/orders/${order.orderId}`)}
                    className="absolute top-5 right-5 py-2 px-2 rounded-md text-xs text-green-700 border-1 border-gray-400"
                  >
                    View Details
                  </button>
                </div>
